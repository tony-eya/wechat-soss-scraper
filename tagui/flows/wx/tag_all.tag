...
//   py flow_articles()       点击"文章"标签(缓存坐标) -> 解析首屏 -> 落库 -> 状态机
//
// 侦查层(recon)保证每个图标先侦查确认可定位;坐标全部运行时定位,不写死 ->
// 开源安全。input_field/article 由 py 块 get_input_field / get_article_coord
// 侦查(缓存复用)后以 SendInput 真实点击完成(微信 UI 只响应真实输入);
// home 仍由 tag 原生 click 完成;py 块负责侦查/粘贴/OCR定位/滚动/详情等逻辑。
//
// 兼容两条入口路径:
...
import wx_article
_r = json.loads(wx_article.flow_begin())
print('FLOW_BEGIN ok=%s' % _r.get('ok'))
print('SOSS_ALREADY_OPEN=%s' % bool(wx_article.find_wx_window()))
py finish
...
import wx_article
_r = json.loads(wx_article.flow_wait_soss())
...
sys.path.insert(0, os.environ.get('WX_WORKDIR', ''))
import wx_article
_r = json.loads(wx_article.flow_fill_search())
...
sys.path.insert(0, os.environ.get('WX_WORKDIR', ''))
import wx_article
_r = json.loads(wx_article.flow_articles())
